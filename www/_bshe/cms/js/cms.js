jQuery(document).ready( function() {
	...
	jQuery(".bshe_cms_img").contextMenu({
		menu: 'bshe_cmsimg_menu'
	},
	function(action, el, pos) {
		...
		if (action == 'editer') {
			// エディタ起動
			//var e = new SexyLightBox({find:'sexywhite',color:'white', OverlayStyles:{'background-color':'#000'}, imagesdir:bshe_basedir + '/sexy-lightbox-2/sexyimages'});
			SexyLightbox.show('編集',  bshe_basedir +'/cms/image/noinc-uploader.html?elementClass=' +
				jQuery('#' + jQuery(el).attr('id')).attr('class') +
				'&elementId=' + elementId +
				'&pageId=' + bshe_templatename +
				'TB_iframe=true&height=210' +
				'&width=396', 'sexylightbox');
        } else if (action == 'publish') {
        	// 保存公開
        	document.getElementById(elementId).publishImageModule();
        } else if (action == 'undo') {
        	// 元に戻す
        	document.getElementById(elementId).undoImageModule();
        } else if (action == 'menu') {
        	// メニュー
        	location.href = bshe_basedir + '/cms/admin/index.html';
        } else if (action == 'logout') {
